...
def email_to_pandas(client: IMAPClient, folder: str = "INBOX") -> pd.DataFrame:
    ...
    emails[["subject", "from", "date"]] = emails["message"].apply(
        lambda x: pd.Series(breakout_email_columns(x))
    )
    return emails
...
def breakout_email_columns(raw_message) -> list:
    ...
    if isinstance(subject, bytes):
        try:
            ...
            print(
                f"Could not decode header for message: {subject}, from: {message.get('From')}"
            )
